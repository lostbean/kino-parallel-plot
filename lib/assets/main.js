...
export function init(ctx, data_info) {
  ...
  function renderLegend() {
    ...
    const legendHoverHandler = function (selected) {
      d3.select("g.active")
        .selectAll("path")
        .style("display", (d) =>
    ...
    legend
      ...
      .text((d) => d[label_col])
      .on("mouseover", (ev, d) => legendHoverHandler(d))
  ...
  const groupHoverHandler = function (group) {
    d3.select("g.active")
      .selectAll("path")
      .style("display", (d) => (d[group_col] === group[0] ? null : "none"));
  };
  function renderGroupLabels() {
    ...
    groupSvg
      ...
      .text((d) => d[0])
      .on("mouseover", (ev, d) => groupHoverHandler(d))
      .on("mouseout", applyFilters);
  }
